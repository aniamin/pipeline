...
func (b *AzureObjectStore) CreateBucket(bucketName string) {
	...
	searchCriteria := b.newManagedBucketSearchCriteriaByRG(generateResourceGroupName(b.location))
	...
	if b.resourceGroup == "" {
		if managedBucket.ResourceGroup == "" {
			managedBucket.ResourceGroup = generateResourceGroupName(b.location)
		}
		b.resourceGroup = managedBucket.ResourceGroup
	}
	if b.storageAccount == "" {
		if managedBucket.StorageAccount == "" {
			managedBucket.StorageAccount = generateStorageAccountName()
		}
		b.storageAccount = managedBucket.StorageAccount
	}
	// We need to zero the ID otherwise the wrong record get updated instead of
	// a new record is inserted
	if managedBucket.Name != bucketName {
		managedBucket.ID = 0
	}
	...
	log.Infof("Creating resourcegroup %s", b.resourceGroup)
	...
	log.Infof("Resourcegroup %s created", b.resourceGroup)
...
func (b *AzureObjectStore) DeleteBucket(bucketName string) error {
	...
	managedBucket := &ManagedAzureBlobStore{}
	searchCriteria := b.newManagedBucketSearchCriteriaByName(bucketName)
...
// newManagedBucketSearchCriteriaByName returns the database search criteria to find managed bucket with the given name
// within the scope of the specified resource group and storage account
func (b *AzureObjectStore) newManagedBucketSearchCriteriaByName(bucketName string) *ManagedAzureBlobStore {
...
// newManagedBucketSearchCriteriaByRG returns the database search criteria to
// find managed bucket within the given resource group
func (b *AzureObjectStore) newManagedBucketSearchCriteriaByRG(resourceGroupName string) *ManagedAzureBlobStore {
	return &ManagedAzureBlobStore{
		OrgID:          b.org.ID,
...
func generateStorageAccountName() string {
	return strings.Replace(uuid.NewV4().String(), "-", "", -1)[:22]
}
